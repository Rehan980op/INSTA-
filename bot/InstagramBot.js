...
class InstagramBot {
  ...
  async loadAndLogin() {
    ...
    const rawCookies     = hasCookieFile ? fs.readFileSync(config.ACCOUNT_FILE, 'utf-8') : '';
    const cookieData     = this._prepareCookieContent(rawCookies);

    if (cookieData.valid) {
      logger.info(`Loading cookies from account.txt (${cookieData.format}; keys: ${cookieData.keys.join(', ')})…`);
      this.ig = await login(cookieData.header);
    } else if (hasCredentials) {
      logger.info(`No usable cookies found (${cookieData.reason}) — logging in with email/password…`);
      this.ig = await login({
      ...
      throw new Error(
        `No usable cookies in account.txt (${cookieData.reason}). ` +
        'Add a fresh FULL Instagram cookie to account.txt or fill in instagramAccount.email/password in config/default.json.'
      );
    }
  ...
  _prepareCookieContent(content) {
    const parsed = this._parseCookieContent(content);
    const required = ['sessionid', 'csrftoken', 'ds_user_id'];
    const missing = required.filter(k => !parsed.cookies[k]);

    if (!parsed.header) {
      return { valid: false, reason: 'account.txt is empty or not a cookie file', header: '', keys: [], format: parsed.format };
    }

    if (missing.length) {
      return {
        valid: false,
        reason: `missing required cookie(s): ${missing.join(', ')}`,
        header: parsed.header,
        keys: Object.keys(parsed.cookies),
  ...
  _parseCookieContent(content) {
    ...
    if (!raw) {
      return { cookies, header: '', format: 'empty' };
    }

    // Cookie-Editor / cookies.txt Netscape export:
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 7) {
        format = 'netscape';
        addCookie(parts[5], parts.slice(6).join('\t'));
      }
    }

    // Browser request-header style: "Cookie: a=b; c=d" or "a=b; c=d"
    if (!Object.keys(cookies).length) {
      format = raw.toLowerCase().startsWith('cookie:') ? 'cookie-header-with-prefix' : 'cookie-header';
      let headerText = raw.replace(/^cookie:\s*/i, '');
      headerText = headerText.replace(/\r?\n/g, '; ');
      for (const pair of headerText.split(';')) {
        const idx = pair.indexOf('=');
        if (idx === -1) continue;
        addCookie(pair.slice(0, idx), pair.slice(idx + 1));
      }
    }
  ...
  _handleListenFailure(err) {
    ...
    if (isAuthError) {
      this.isRunning = false;
      logger.error('Session expired or invalid. Update account.txt with a fresh FULL Instagram cookie, then restart.');
  ...
  startListening() {
    ...
    try {
      const listener = this.ig.listen((err, event) => {
        if (err) {
          this._handleListenFailure(err);
          return;
        }
      ...
      if (listener && typeof listener.catch === 'function') {
        listener.catch(err => this._handleListenFailure(err));
      }
    } catch (err) {
      this._handleListenFailure(err);
      return;
    ...
    return {
      sendMessage: async (text, threadID) => {
        try {
          ...
          logger.error('Failed to send message', { error: msg, threadID });
          if (/login_required|not authorized|unauthorized/i.test(msg)) {
            this.isRunning = false;
            logger.error('Send failed because Instagram rejected the session. Refresh account.txt with a fresh FULL cookie.');
          }
          throw error;
        }
      },
