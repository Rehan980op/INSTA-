...
Minimum required cookies: `sessionid`, `csrftoken`, `ds_user_id`. Full cookie export is best: `sessionid`, `csrftoken`, `ds_user_id`, `mid`, `ig_did`, `rur`.

Before running, you can check the cookie file:
```bash
npm run check-cookie
```

If the bot still shows `login_required` or `Connection refused: Not authorized`, Instagram rejected the session. Log in to Instagram in your browser/app, approve any security alert, export a fresh full cookie, and replace `account.txt`.
...
├── account.txt               ← Instagram cookies (Netscape or one-line header format) — keep secret!
├── config/
